...
| AI2D_TEST | 79.24 | 79.27 | 79.27 | ±1.43 |
...
### 2.7.1 幻觉专项：POPE 与 HallusionBench

上面四项都是 Yes/No 或选择题，而 RLAIF-V 针对的是**生成式幻觉**。
为排除「尺子不对口」，补测两个幻觉专项。

**POPE**（5127 题，71% 问不存在的物体）——按题级统计，
每题只算一次（`tools/compare_pope.py`）：

| 指标 | baseline | DPO 5e-6 | DPO 2e-5 |
|---|---|---|---|
| 幻觉率（不存在物体答 Yes） | **10.97%** | 11.00% | 11.11% |
| precision | 77.18 | 77.13 | 76.96 |
| recall | 89.73 | 89.73 | 89.73 |
| 假阳性数 | 398 | 399 | 403 |
| McNemar p | — | 1.000 (5 题变化) | 0.267 (13 题变化) |

> **口径提醒**：VLMEvalKit 自己报的 precision 是 88.55，
> 与这里的 77.18 不同——它把多标签题（random/popular/adversarial）
> 展开成多行，一道三标签的题会被计三次。分 split 看没问题，
> 但算总体幻觉率时每题应当只计一次。
...
| 指标 | baseline | DPO 5e-6 | DPO 2e-5 |
|---|---|---|---|
| aAcc | 61.93 | 61.93 | 61.83 |
| fAcc | 37.57 | 37.57 | 37.28 |
| qAcc | 35.60 | 35.60 | 35.60 |

5e-6 组三个指标与 baseline **逐位相同**。
