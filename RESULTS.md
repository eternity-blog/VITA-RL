...
## 6. Before / after（配置 A，LR 5e-6）

### MME

| 指标 | baseline | DPO 后 | Δ |
|---|---|---|---|
| perception | 1728.86 | 1729.88 | +1.02 |
| reasoning | 624.64 | 622.50 | −2.14 |
| **TOTAL** | **2353.51** | **2352.38** | **−1.12（−0.05%）** |

**14 个子项里 12 个逐位相同**，只有两项有变化：
`posters` +1.02、`commonsense_reasoning` −2.14。

这种「几乎完全相同」本身就是证据链的一环：它与第 5 节测得的
**策略仅漂移 0.27% 序列 log-prob** 完全吻合。
LoRA 确实被训练了、adapter 确实被合并了（12 项相同不代表没加载——
若没加载则应当 14 项全同，且 `posters` 那 +1.02 不会出现），
但扰动量太小，不足以改变绝大多数题目的答案。
