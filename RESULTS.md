...
### 训练中期观察：噪声确实降了，但漂移量没变

跑到 44/238 步（有效 batch 63）时的三组对比：

| 配置 | 数据量 | 有效 batch | grad_norm | accuracy | margin | **策略漂移** |
|---|---|---|---|---|---|---|
| A | 3000 | 16 | 0.34–0.45 | 0.520 | +0.0026 | 0.269% |
| B | 3000 | 16 | 0.34–0.45 | 0.539 | +0.0047 | 0.372% |
| C | 15000 | **63** | **0.20–0.24** | **0.609** | **+0.0096** | 0.293% |

**大 batch 起作用了，而且是按预期的方式起作用的**：

- `grad_norm` 从 0.34–0.45 降到 0.20–0.24 —— 平均 63 个样本后
  梯度噪声确实降下来了，这正是 √G 该有的效果。
- accuracy 0.609、margin 是 A 组的 3.7 倍 —— 判别方向学得更准了。

**但第 6 列才是关键**：策略漂移 0.293%，和 A 组的 0.269% 几乎一样。

> **「学得更准」和「改变得更多」是两件事。**
> DPO 的更新量正比于 `beta × 梯度`，梯度方向变准了不等于步长变大了。
> benchmark 响应的是漂移量，不是 margin。
> 所以**训练指标好看，不代表评测会动** —— 这一条在 B 组
> （训练更好、评测持平）已经出现过一次，C 组是第二次印证。
