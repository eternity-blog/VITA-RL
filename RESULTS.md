...
| 指标 | baseline | C 组（15000/batch63） | Δ |
|---|---|---|---|
| perception | 1728.86 | 1731.25 | +2.38 |
| reasoning | 624.64 | 615.00 | −9.64 |
| **TOTAL** | **2353.51** | **2346.25** | **−7.26** |

2374 道题里 **11 道答案改变**（A 组只有 2 类子项动过），
说明 6 倍的漂移确实让模型动得更多了——**但方向不好**：
`numerical_calculation` −7.5、`commonsense_reasoning` −2.14。

**这正是「漂移变大」的双面性**：0.27% 时模型几乎没变，
1.58% 时它真的变了，但变化的方向由那个信噪比 0.055 的
弱信号决定——**信号弱意味着学到的方向里噪声占主导，
所以放大它并不会让模型变好，只会让它更多地偏离原本
校准良好的行为。**

> 换句话说：前两轮的「没变化」不是坏运气，
> 而是**弱信号下唯一安全的结果**。C 组证明了
> 加大 batch 确实能让 DPO 在这批数据上「学起来」，
> 也同时证明了**在信噪比不足时，学起来不等于变好**。
