...
**基座模型本来就分不开这些样本**——53% 约等于抛硬币，
而且 log-prob 差的均值 +0.48 完全被 45 的标准差淹没。

DPO 的梯度正比于「模型当前错得有多系统」。当基座对 chosen/rejected
的打分本质上是随机的，就不存在一个一致的方向可供优化，
**margin 平坦是数据的性质，不是 trainer 的 bug**。
...
- **接近 50%** → 数据对这个基座太难。要么换更容易的偏好数据，
  要么先 SFT 提升基座在该分布上的能力，再做偏好优化。
- **明显高于 50%（如 65%+）** → 信号存在，此时 margin 还平坦
  才该去查 trainer、学习率、beta。
